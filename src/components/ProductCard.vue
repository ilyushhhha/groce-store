<template>
  <div class="bg-white rounded-xl shadow-xl transition-transform transform hover:scale-90 cursor-pointer overflow-hidden m-2 w-1/3 sm:w-1/3 md:w-1/4"
       @click="openPopup(product)">
    <img :src="product.images[0].small_image_url" alt="Изображение продукта" class="w-full h-48 object-cover rounded-t-xl" />
    <div class="p-4">
      <h3 class="text-xl font-bold text-gray-800 mb-1">{{ product.name }}</h3>
      <div class="flex items-center mb-2">
        <span class="text-yellow-500">
          <template v-for="star in 5" :key="star">
            <svg v-if="star <= product.ratings.average" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 inline" fill="currentColor" viewBox="0 0 20 20">
