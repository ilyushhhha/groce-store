<template>
  <div
    ...
    <img :src="product.imageUrl" alt="Изображение продукта" class="w-full h-48 object-cover rounded-t-xl" />
    ...
    <div class="p-4">
      ...
      <div class="flex items-center mb-2">
        <span class="text-yellow-500">
          <template v-for="star in 5" :key="star">
            <svg v-if="star <= product.rating" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 inline" fill="currentColor" viewBox="0 0 20 20">
      ...
      <p class="text-lg font-semibold text-gray-900">{{ product.price }} {{ product.size }}</p>
      <p class="text-sm text-gray-500">{{ product.concentration }} | {{ product.gender }}</p>
    </div>
  </div>
</template>
