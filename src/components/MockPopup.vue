    ...
    <div v-if="popupVisible" class="fixed inset-x-0 bottom-0 z-50 flex justify-center items-end">
      <div class="bg-white h-1/2 w-full rounded-t-2xl p-10 relative shadow-2xl">
        ...
        <div class="flex flex-col md:flex-row items-center mb-6">
          <img :src="selectedProduct.imageUrl" alt="Изображение продукта" class="w-40 h-40 object-cover rounded-lg mb-4 md:mb-0 md:mr-6" />
          <div>
            ...
            <div class="flex flex-col mb-4">
              ...
              <p class="text-gray-600">Ноты: {{ selectedProduct.fragranceNotes.top }} (верхние), {{ selectedProduct.fragranceNotes.middle }} (средние), {{ selectedProduct.fragranceNotes.base }} (базовые)</p>
            </div>
        ...
        <button @click="handleAddToCart(selectedProduct, $event)" class="w-full bg-gray-900 text-white py-3 rounded-lg text-xl hover:bg-gray-700 focus:bg-red-500 transition duration-300 ease-in-out">
          Добавить в корзину
        </button>
      </div>
    </div>
